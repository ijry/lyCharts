...
const drawGauge = (ctx) => {
	...
	const values = data.map((item, index) => {
		if (isInited.value && progressValue.value[index] !== undefined) {
			return progressValue.value[index];
		}
		return item.value || 0;
	});
	...
	// 使用新的角度转弧度函数
	let startRad = angleToRadBase(startAngle);
	let endRad = angleToRadBase(endAngle);
	// 修复角度计算逻辑，确保按顺时针方向计算
	...
	if (Array.isArray(lineStyleColor) && lineStyleColor.length > 0) {
		...
		for (let i = 0; i < lineStyleColor.length; i++) {
			...
			const segmentStartAngle = angleToRadBase(startAngle - startPercent * totalAngle);
			const segmentEndAngle = angleToRadBase(startAngle - percent * totalAngle);
	...
	if (progress.show !== false) {
		...
		data.forEach((item, index) => {
			...
			const progressEndAngle = angleToRadBase(startAngle - clampedProgress * totalAngle);
		...
		for (let i = 0; i <= splitNumber; i++) {
			...
			if (i < splitNumber) { // 不在最后一个分割线后绘制刻度线
				for (let j = 1; j <= axisTickNumber; j++) {
					// 计算每个刻度线的角度位置
					const angle = angleToRadBase(startAngle - (i + j / (axisTickNumber + 1)) / splitNumber * totalAngle);
	...
	if (splitLine.show !== false) {
		...
		for (let i = 0; i <= splitNumber; i++) {
			...
			const angle = angleToRadBase(startAngle - (i / splitNumber) * totalAngle);
	...
	if (data.length > 0 && pointer.show !== false) {
		...
		data.forEach((item, index) => {
			...
			const progressPercent = (value - min) / (max - min);
			const clampedProgress = Math.max(0, Math.min(1, progressPercent)); // 限制在0-1之间
			const pointerAngle = angleToRadBase(startAngle - clampedProgress * totalAngle);
 ...
 * @created 2025-08-05
 */
const startAnimation = (targetValue, duration = 1000) => {
	// 清除之前的动画
	...
	const startValue = progressValue.value;
	...
	animationTimer.value = setInterval(() => {
		...
		progressValue.value = startValue + diff * easeOutQuart;
...
const updateChart = (option) => {
	...
	if (detail.valueAnimation) {
		...
		// 修改: 为每个数据项创建动画
		const startValues = progressValue.value.length > 0 ? progressValue.value : new Array(values.length).fill(0);
		...
		const animate = () => {
			...
			// 修改: 为每个数据项计算进度值
			progressValue.value = startValues.map((start, index) => {
